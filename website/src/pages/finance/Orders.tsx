...
import { Search, Filter, Eye, DollarSign, Download, Check, AlertCircle } from 'lucide-react';
            ...
            <div className="space-y-6">
              ...
              {paymentStatus[selectedOrder.id] === 'unpaid' ? (
                <div className="border-t border-gray-100 pt-5 space-y-3">
                  <div className="bg-red-50/50 border border-red-100 p-3 rounded-lg flex gap-2 text-xs text-red-600">
                    <AlertCircle size={15} className="flex-shrink-0 mt-0.5" />
                    <span>Payment receipt check pending for this transaction. Confirm deposit below.</span>
                  </div>
                  <button
                    onClick={() => confirmPayment(selectedOrder.id)}
                    className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-all"
                  >
                    <Check size={14} /> Verify Settlement Deposit
                  </button>
                </div>
              ) : (
